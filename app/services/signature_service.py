...
class SignatureService:
    ...
    def __init__(self):
        """Initialize signature service with mTLS configuration."""
        self.api_url = settings.vendor_api_url
        self.cert_path = settings.vendor_mtls_cert_path
        self.key_path = settings.vendor_mtls_key_path
        self.ca_path = settings.vendor_mtls_ca_path
        self.timeout = settings.request_timeout

        logger.info(f"Signature service initialized for API: {self.api_url}")
        logger.info(f"Using mTLS certificate: {self.cert_path}")
